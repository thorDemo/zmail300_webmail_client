...
def thread_mission(proxies):
    log.warning(f'CONNECT EMAIL SERVER ')
    temp = 0
    while temp < 5:
        while True:
            try:
                ...
                if server.x_token is None:
                    log.warning(f'{temp}-Login Web Mail Failed:{server.message} Retry Waiting 5s')
                    time.sleep(5)
                    continue
                else:
                    log.warning(f'{temp}-Login Web Mail Success:{username, password}')
                    post_auth_user(username, password)
                    log.warning(f'{temp}-Send Back Auth Account:{username, password}')
                    break
            except RequestException:

                log.warning(f'{temp}-Request Server Exception Retry Waiting 5s')
                temp += 1
                time.sleep(1)
        try:
            ...
            log.warning(f"{temp}-Send Mail To {mission_data['receivers']}")
            ...
            if 'Permission denied' in result:
                log.warning(f"{temp}-Account Error Retry Waiting 5s")
                time.sleep(5)
                continue
            # 不存入发件箱 不需要删除了
            log.warning(f"{temp}-Send Success:{result} Delay: {mission_data['delay']}")
